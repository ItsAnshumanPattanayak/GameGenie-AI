...
The protected React routes `/dashboard`, `/history`, and `/favourites` provide loading, empty, and error states. The Dashboard includes the primary natural-language recommendation form; a successful authenticated request displays results and reloads its recent-search summary from the persisted API data. History supports repeat search and deletion. Favourites support catalogue/generator links and optimistic removal with rollback. Recommendation cards optimistically add/remove favourites and roll back with a readable error if the request fails; they also submit all four feedback types.
